...
class DeviceProcessor {
 public:
  ...
  ~DeviceProcessor() { RC_LOG_INFO("DeviceProcessor destroyed: %s", device_info_.sn.c_str()); }
...
class PostProcessNode : public Node {
 public:
  PostProcessNode() : Node("postprocess_node") {
    ...
    // Start listening for device_info messages
 ...
 private:
  void StartDeviceDiscovery() {
#if ROS_VERSION_MAJOR == 1
    // Subscribe to device_info topic for multi-device support
    // Use larger queue and transport hints to ensure all messages are received
    device_info_sub_ = nh().subscribe<std_msgs::String>(
        "odin_driver/device_info", 100,
    ...
    resolution_change_sub_ = nh().subscribe<std_msgs::String>(
        "odin_driver/resolution_change", 10,
        [this](const boost::shared_ptr<std_msgs::String const>& msg) {
          OnResolutionChange(msg->data);
        });
#else
    // Subscribe to device_info topic with transient_local QoS to receive latched messages
    device_info_sub_ = create_subscription<std_msgs::msg::String>(
        "odin_driver/device_info", rclcpp::QoS(10).transient_local(),
  ...
  void OnDeviceInfoReceived(const std::string& json) {
    RC_LOG_INFO("Received device_info: %s", json.c_str());

    DeviceInfo info;
    if (!ParseDeviceInfoJson(json, info)) {
      RC_LOG_WARN("Failed to parse device_info JSON: %s", json.c_str());
      return;
    }
...
#if ROS_VERSION_MAJOR == 1
  ros::Subscriber device_info_sub_;
  ros::Subscriber device_offline_sub_;
  ros::Subscriber resolution_change_sub_;
#else
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr device_info_sub_;
...
void DeviceProcessor::SetupPubSub() {
  ...
  cloud_pub_ = parent_->advertise<PointCloud2>(prefix + "cloud/render", 10);
  image_pub_ = parent_->advertise<Image>(prefix + "camera0/raw", 10);
  image2_pub_ = parent_->advertise<Image>(prefix + "camera1/raw", 10);
  image_undistort_pub_ = parent_->advertise<Image>(prefix + "camera0/undistort", 10);
  image2_undistort_pub_ = parent_->advertise<Image>(prefix + "camera1/undistort", 10);
  left_rect_pub_ = parent_->advertise<Image>(prefix + "camera0/rect", 10);
  right_rect_pub_ = parent_->advertise<Image>(prefix + "camera1/rect", 10);
  left_info_pub_ = parent_->advertise<CameraInfo>(prefix + "camera0/camera_info", 10);
  right_info_pub_ = parent_->advertise<CameraInfo>(prefix + "camera1/camera_info", 10);
  depth_cloud_pub_ = parent_->advertise<PointCloud2>(prefix + "stereo/depth", 10);

  image_sub_ = parent_->subscribe<CompressedImage>(
      prefix + "camera0/compressed", 1,
      [this](const CompressedImageConstPtr& msg) { imageCallback(msg); });
  image2_sub_ = parent_->subscribe<CompressedImage>(
      prefix + "camera1/compressed", 1,
      [this](const CompressedImageConstPtr& msg) { imageCallback2(msg); });
  cloud_sub_ = parent_->subscribe<PointCloud2>(
      prefix + "cloud/raw", 1, [this](const PointCloud2ConstPtr& msg) { cloudCallback(msg); });
  disparity_sub_ = parent_->subscribe<DisparityImage>(
      prefix + "stereo/disparity", 1,
