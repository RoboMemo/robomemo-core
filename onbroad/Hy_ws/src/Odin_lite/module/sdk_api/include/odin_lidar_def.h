...
enum class OdomSourceType : uint8_t {
  kLowFrequency = 0,   // Low-frequency odometry (e.g., GPS/visual odometry)
  kHighFrequency = 1,  // High-frequency odometry (e.g., IMU pre-integration)
  kTransform = 2,      // Transform directly from TF tree
  kCalibration = 3,    // Temporary data during calibration
  kWorldToCamera = 4   // World-to-camera transform (formerly R_wt_wc)
};
...
static_assert(sizeof(OdinImuSample) == 24, "Unexpected OdinImuSample size");
...
struct DiscoveredDevice {
  std::string ip;                ///< Device IP/address (IP for network, path for USB)
