...
void ConnectAndRegister(DeviceContext* ctx) {
  std::cout << "[" << GetTimestamp() << "] Connecting to device: " << ctx->device.ip
...
void DisconnectAndRelease(DeviceContext* ctx) {
  ...
  std::cout << "[" << GetTimestamp() << "] Disconnecting device: " << ctx->device.ip
            << " (SN: " << ctx->device.sn << ")" << std::endl;
...
void OnDeviceAttach(const DiscoveredDevice& device) {
  std::string key = device.sn.empty() ? device.ip : device.sn;
...
void PrintStatistics() {
  ...
  for (auto& kv : g_devices) {
    ...
    std::cout << "Device: " << ctx->device.sn << " (" << ctx->device.ip << ")\n";
