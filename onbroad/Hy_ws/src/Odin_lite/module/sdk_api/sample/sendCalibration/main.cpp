...
void PrintUsage(const char* program_name) {
  std::cout << "Usage: " << program_name << " <calibration_file> [options]\n"
            << "\n"
            << "Send calibration file to ODIN device.\n"
            << "\n"
            << "Arguments:\n"
            << "  <calibration_file>  Path to the calibration file to send\n"
            << "\n"
            << "Options:\n"
...
int main(int argc, char* argv[]) {
  ...
  if (file_path.empty()) {
    std::cerr << "Error: calibration_file is required.\n";
    PrintUsage(argv[0]);
    return 1;
  }
  ...
  std::cout << "========================================\n";
  std::cout << "\n";
  std::cout << "ODIN Send Calibration File Tool\n";
  std::cout << "Calibration file: " << file_path << "\n";
  ...
  for (size_t i = 0; i < devices.size(); ++i) {
    std::cout << "  [" << i << "] " << devices[i].ip << " (SN: " << devices[i].sn << ")\n";
  }
  ...
  std::cout << "Using device: " << target_device->ip << " (SN: " << target_device->sn << ")\n";
  ...
  // Send calibration file
  std::cout << "Sending calibration file...\n";

  OdinResult result =
      SendFileToDevice(handle, ProgressCallback, OdinFileType::kCalibrationFile, file_path);
  std::cout << "Finish sending calibration file...\n";
  if (result != OdinResult::kOk) {
    std::cerr << "Failed to send calibration file. Error code: " << static_cast<int>(result)
              << "\n";
  ...
  if (g_transfer_success.load()) {
    std::cout << "\n========================================\n";
    std::cout << "Calibration file sent successfully: " << file_path << "\n";
    std::cout << "========================================\n";
    return 0;
  } else {
